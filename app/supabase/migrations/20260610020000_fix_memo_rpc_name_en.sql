-- [hotfix] 이슈 025(20260609000001)가 메모 RPC 재정의 시 sectors JSON에서 name_en을 누락한 회귀 복원
--
-- 영향: list_memos / create_memo_with_links / update_memo_with_links 응답의
--       sectors[].name_en — FE(types/memo.ts, StockDetailSheet 등)가 이 필드를 사용한다.
-- 내용: 025의 최신 함수 정의에 'name_en', se.name_en 한 줄씩만 추가 (그 외 변경 없음).
