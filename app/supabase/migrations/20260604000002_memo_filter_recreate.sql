-- [014] list_memos 함수 재생성 — PostgREST 스키마 캐시 강제 갱신
-- 구 시그니처 DROP 후 새 시그니처로 재생성하여
-- PostgREST가 새 함수(p_stock_ids uuid[], p_sector_ids int[])를 인식하도록 함

-- 혹시 남아있을 수 있는 구 시그니처 변형도 모두 DROP
drop function if exists public.list_memos(date, date, uuid, boolean, boolean, boolean, int, boolean, int, int);
drop function if exists public.list_memos(date, date, uuid[], boolean, boolean, boolean, int[], boolean, int, int);
