...
사용자의 카테고리 목록을 소속 종목 수와 함께 조회한다. 생성순(`created_at` asc)으로 반환한다.

- **방식**: REST (auto-generated)
- **호출**: `supabase.from('user_categories').select('*, user_category_stocks(count)').eq('user_id', userId).order('created_at', { ascending: true })`
- **인증**: 필요
...
Array<{
  ...
  created_at: string
  updated_at: string
  user_category_stocks: Array<{ count: number }>   // 소속 종목 수
}>
```
...
새 카테고리를 생성한다. 같은 사용자 내 동일 이름은 UNIQUE 제약으로 차단된다.

- **방식**: REST (auto-generated)
- **호출**: `supabase.from('user_categories').insert({ user_id, name })`
- **인증**: 필요

**요청 파라미터**
| 파라미터 | 타입 | 필수 | 설명 |
|---------|------|------|------|
| user_id | string (uuid) | Y | 사용자 ID |
| name | string | Y | 카테고리 이름 (예: '2차전지', 'AI반도체') |
...
| 401 | 인증 토큰 없음 또는 만료 |
| 403 | RLS 위반 (다른 사용자 user_id로 삽입 시도) |
| 409 | 동일 이름 카테고리 중복 (UNIQUE 제약 위반) |
...
- **방식**: REST (auto-generated)
- **호출**: `supabase.from('user_categories').update({ name, updated_at: new Date() }).eq('id', categoryId)`
- **인증**: 필요

**요청 파라미터**
| 파라미터 | 타입 | 필수 | 설명 |
|---------|------|------|------|
| id | string (uuid) | Y | 수정할 카테고리 ID |
| name | string | Y | 변경할 카테고리 이름 |
...
|------|------|
| 401 | 인증 토큰 없음 또는 만료 |
| 403 | RLS 위반 (다른 사용자 카테고리 수정 시도) |
| 404 | 해당 ID 카테고리 없음 |
| 409 | 변경할 이름이 이미 존재 (UNIQUE 제약 위반) |

---

### 카테고리 삭제

카테고리를 삭제한다. `user_category_stocks`, `memo_categories`는 cascade 삭제되며 메모 본체는 유지된다.

- **방식**: REST (auto-generated)
- **호출**: `supabase.from('user_categories').delete().eq('id', categoryId)`
- **인증**: 필요

**요청 파라미터**
| 파라미터 | 타입 | 필수 | 설명 |
|---------|------|------|------|
| id | string (uuid) | Y | 삭제할 카테고리 ID |
...
- **방식**: REST (auto-generated)
- **호출**: `supabase.from('user_category_stocks').select('*, stocks(*)').eq('category_id', categoryId)`
- **인증**: 필요

**요청 파라미터**
| 파라미터 | 타입 | 필수 | 설명 |
|---------|------|------|------|
| category_id | string (uuid) | Y | 카테고리 ID |
...
|------|------|
| 401 | 인증 토큰 없음 또는 만료 |
| 403 | RLS 위반 (다른 사용자 카테고리 조회 시도) |

---

### 카테고리에 종목 추가

카테고리에 종목을 추가한다. 이미 추가된 종목은 PK 중복으로 차단된다.

- **방식**: REST (auto-generated)
- **호출**: `supabase.from('user_category_stocks').insert({ category_id, stock_id })`
- **인증**: 필요
...
| 코드 | 상황 |
|------|------|
| 400 | 이미 카테고리에 포함된 종목 중복 삽입 |
| 401 | 인증 토큰 없음 또는 만료 |
| 403 | RLS 위반 (다른 사용자 카테고리에 종목 추가 시도) |
| 404 | category_id 또는 stock_id가 존재하지 않음 |

---

### 카테고리에서 종목 제거

카테고리에서 종목을 제거한다. 종목 자체 및 카테고리는 유지된다.

- **방식**: REST (auto-generated)
- **호출**: `supabase.from('user_category_stocks').delete().eq('category_id', categoryId).eq('stock_id', stockId)`
- **인증**: 필요
...
|---------|------|------|------|
| category_id | string (uuid) | Y | 카테고리 ID |
| stock_id | string (uuid) | Y | 제거할 종목 ID |
...
|------|------|
| 401 | 인증 토큰 없음 또는 만료 |
| 403 | RLS 위반 (다른 사용자 카테고리에서 종목 제거 시도) |
| 404 | 해당 카테고리-종목 연결이 존재하지 않음 |
...
- **방식**: RPC (DB Function)
- **호출**: `supabase.rpc('create_memo_with_links', { p_body, p_stocks, p_trade_event_ids, p_news_ids, p_sector_ids, p_category_ids })`
- **인증**: 필요
...
| p_category_ids | `string[] (uuid) \| null` | N | 연결할 카테고리 ID 목록. 빈 배열 또는 null이면 연결 없음. |
...
| 401 | 인증 토큰 없음 또는 만료 |
| 404 | p_stocks의 stock_id, p_trade_event_ids의 event_id, p_news_ids의 news_id, p_sector_ids의 sector_id, p_category_ids의 category_id가 존재하지 않음 |
...
**필터 결합 규칙**
- **같은 행(종목 행 내, 섹터 행 내, 카테고리 행 내) 복수 선택 → OR**: `p_stock_ids`에 여러 종목 지정 시 둘 중 하나라도 연관된 메모를 모두 반환. `p_sector_ids`, `p_category_ids`도 동일.
- **다른 행/타입 간 → AND**: 토글(매매 이벤트·뉴스 연관) + 종목 행 + 섹터 행 + 카테고리 행이 모두 지정된 경우 교집합만 반환. 예: 종목 필터 + `p_trade_events_only: true` 동시 지정 시 "해당 종목 관련 메모 중 매매이벤트도 연결된 것"만 반환.
- **연결 없음(`p_no_links`) 필터는 다른 필터와 상호 배타적**: `p_no_links: true` 지정 시 다른 모든 엔티티 필터를 무시하고 연결 없는 메모만 반환. 종목·섹터·카테고리 어떤 것도 연결되지 않은 메모만 반환한다. 카테고리만 연결된 메모는 "연결 없음"에 해당하지 않는다.
...
- **방식**: RPC (DB Function)
- **호출**: `supabase.rpc('list_memos', { p_from, p_to, p_stock_ids, p_trade_events_only, p_news_only, p_sector_ids, p_category_ids, p_no_links, p_limit, p_offset })`
- **인증**: 필요
...
| p_category_ids | string[] (uuid) \| null | N | 카테고리 필터. 지정된 카테고리들 중 하나라도 연관된 메모 반환 (OR). 종목 경로(user_category_stocks 경유 memo_stocks로 해당 카테고리 종목에 연결된 메모) + 직접 연결 경로(memo_categories로 해당 카테고리에 직접 연결된 메모) OR 합산. 카테고리는 플랫 구조이므로 재귀 CTE 불필요. 다른 타입 필터와 AND 결합. |
| p_no_links | boolean | N | true이면 어떤 엔티티에도 연결되지 않은 메모만 반환 (기본 false). true 지정 시 다른 엔티티 필터는 무시. 카테고리만 연결된 메모는 "연결 없음"에 해당하지 않는다. |
...
- **방식**: REST (auto-generated)
- **호출**: `supabase.from('memos').select('*, memo_stocks(stock_id, goal_price, stocks(id, ticker, name, market, currency, is_active)), memo_trade_events(event_id, account_events(id, event_type, event_date, ticker, name)), memo_news(news_id), memo_sectors(sector_id, sectors(id, code, name, level)), memo_categories(category_id, user_categories(id, name))').eq('id', memoId).single()`
- **인증**: 필요
...
- **방식**: RPC (DB Function)
- **호출**: `supabase.rpc('update_memo_with_links', { p_memo_id, p_body, p_stocks, p_trade_event_ids, p_news_ids, p_sector_ids, p_category_ids })`
- **인증**: 필요
...
| p_category_ids | `string[] (uuid) \| null` | N | 카테고리 연결 전체 교체. null이면 변경하지 않음. 빈 배열이면 모든 카테고리 연결 해제. |
...
- **방식**: REST (auto-generated)
- **호출**: `supabase.from('memo_categories').insert({ memo_id, category_id })`
- **인증**: 필요
...
| 404 | memo_id, stock_id, event_id, news_id, sector_id, category_id가 존재하지 않음 |
...
- **방식**: REST (auto-generated)
- **호출**: `supabase.from('memo_categories').delete().eq('memo_id', memoId).eq('category_id', categoryId)`
- **인증**: 필요
