...
1. **종목 경로**: 지정된 섹터의 하위 레벨에 속하는 종목(`stocks.sector_id`)과 연결된 메모(`memo_stocks` 경유).
   ...
   - 탐색 방향: `stocks.sector_id`에서 `sectors.parent_id`를 따라 조상 경로를 역산하거나, 재귀 CTE로 지정된 sector_id의 모든 하위 sector_id 집합을 구한 뒤 `stocks.sector_id IN (descendants)`로 판단.
2. **직접 연결 경로**: `memo_sectors`로 지정된 섹터(및 해당 섹터의 하위 레벨)에 직접 연결된 메모.
   - L2 "반도체및반도체장비" 필터 시: `memo_sectors`에 L2·L3·L4 중 해당 하위가 직접 연결된 메모도 포함.
...
| p_sector_ids | number[] \| null | N | 섹터 필터. 지정된 섹터들 중 하나라도 연관된 메모 반환 (OR). L1~L4 어느 레벨이든 가능. 종목 경로(해당 섹터 하위 종목에 연결된 메모) + 직접 연결 경로(memo_sectors로 직접 연결된 메모) OR 합산. 다른 타입 필터와 AND 결합. |
