...
>
> **2026-06-15 추가:** 메모 작성/편집 화면에서 **기존 매매이벤트를 직접 선택해 연결하는 경로**(§6.4·§6.5)를 구체화. 후보 목록은 본인의 전체 매수/매도 이벤트를 최신순으로 노출하고 종목·기간 검색을 제공한다. 데이터 레이어(`memo_trade_events` 테이블, `create_memo_with_links`/`update_memo_with_links`의 `p_trade_event_ids`, `linkMemoTradeEvent`/`unlinkMemoTradeEvent`)는 이미 구현돼 있으며, 신규 작업은 **메모 측 선택 UI**에 한정된다.
...
- 보기 화면: **달력형**, **리스트형**
- 필터: 종목별 / 매매 연관 / 뉴스 연관 / 섹터별 / 연결 없음 (복수 선택 가능, 같은 타입 내 OR · 타입 간 AND)
- 엔티티 타입별 색상 표시
...
"직접 연결만 보기" 토글은 제공하지 않는다. 대신, 상단 필터에서 "종목 X"와 "매매 연관"을 동시에 선택하면 위 (1)(2) 교집합이 아닌, "종목 X와 관련이 있으면서 매매이벤트에도 연결된 메모"만 노출된다(AND 결합). 매매이벤트 없이 종목에만 연결된 메모까지 보려면 "종목 X"만 선택하면 된다.
...
- **리스트형**: 메모마다 연결된 엔티티를 칩(태그)으로 표시. 칩 배경 = 타입 색, 라벨 = 엔티티 이름. 다중 연결 시 칩 여러 개.
  - **종목 칩 라벨**: 시장(market)에 관계없이 항상 `name`(종목명)을 표시한다. 한국 주식 티커(`005930` 등)는 숫자 코드라 사용자가 식별하기 어렵기 때문에 종목명을 우선한다.
...
필터 영역은 상단에 고정되며, **토글 행 → 종목 행 → 섹터 행** 순서로 세 줄로 구성된다.

#### 토글 행 (최상단)
- **매매 이벤트** 토글 — 매매이벤트에 연결된 메모만 표시
- **뉴스 연관** 토글 — 뉴스에 연결된 메모만 표시
- 두 토글은 독립적으로 ON/OFF 가능하며, 동시에 켤 수 있다(AND 결합).

#### 종목 필터 행
- 행 왼쪽에 작은 헤더 레이블 "종목" 표시.
- 칩 형태의 종목 목록을 가로 스크롤로 나열. 복수 선택 가능.
- 4.1 규칙 적용. "직접 연결만" 토글은 표시하지 않음.

#### 섹터 필터 행
- 행 왼쪽에 작은 헤더 레이블 "섹터" 표시.
- 칩 형태의 섹터 목록을 가로 스크롤로 나열. 복수 선택 가능.
- L1 선택 시 `sectorIds`에 **L1 id + 하위 L2 id 모두 포함**. L2 미로딩 상태에서도 L1 id만으로 즉시 필터 적용 가능 (PRD-004 §7.1 참조).

#### 결합 규칙

- **같은 행 내 복수 선택 → OR**: 종목 A + 종목 B 선택 시 둘 중 하나라도 연관된 메모를 모두 표시. 섹터도 동일.
- **다른 행/타입 간 → AND**: 토글 + 종목 + 섹터가 모두 선택된 경우 교집합만 표시.

예시:
- 종목 A + 종목 B + 매매 이벤트 토글 ON → `(종목 A 관련 OR 종목 B 관련) AND 매매이벤트 연결`
- 섹터 IT + 섹터 금융 + 뉴스 연관 토글 ON → `(IT 섹터 OR 금융 섹터) AND 뉴스 연결`

#### 연결 없음 필터
- 토글 행에 "연결 없음" 토글 추가. 다른 필터와 상호 배타적으로 동작하며, 선택 시 다른 필터는 자동 해제됨.

### 6.4 메모 작성/편집
- 본문 입력 + 엔티티 연결 선택(종목/매매이벤트/뉴스/섹터, 복수 선택).
- 종목 선택 시 목표가 입력 필드 노출(선택 입력).
- **섹터 연결**: L1 → L2 → L3 → L4 cascading select로 원하는 단계까지 선택 후 연결. 반드시 최하위까지 내려갈 필요 없이, 원하는 레벨에서 확정 가능 (PRD-004 §3.4 참조).
- 작성 후 수정·삭제 가능.

#### 매매이벤트 연결 (메모 측 진입)

종목·섹터 연결과 같은 방식으로, 메모 작성/편집 화면에 **"매매이벤트" 행**을 두고 선택 모달로 본인의 매수/매도 이벤트를 복수 연결한다. 기존에는 매수/매도 폼(§6.5)에서 메모를 동시 작성하는 경로만 있었고, 메모 측에서 기존 이벤트를 골라 붙이거나 떼는 경로가 없었다 — 이 경로를 추가한다.

- **후보 범위**: 본인의 `account_events` 중 `event_type IN ('buy', 'sell')`만. 입금/출금/배당은 "매매 이유 기록" 용도가 아니므로 후보에서 제외한다.
- **목록 구성**: 전체 매수/매도 이벤트를 **최신순(`event_date desc`)**으로 나열. 각 항목 표시 = `발생일 · 매수/매도 · 종목명 · 수량 @ 체결가`(통화 포함). 매수/매도는 색상/라벨로 구분.
- **검색/필터**: 이벤트가 많은 사용자를 고려해 모달 상단에 검색·필터를 제공한다.
  - **종목 검색**: 종목명·티커 텍스트 검색(디바운스).
  - **기간 필터**: 시작~종료일 범위 선택(선택 입력).
  - 검색·필터 결과도 최신순 정렬을 유지한다.
- **선택 시드(편의)**: 메모에 이미 연결된 종목이 있으면 그 종목명을 검색어 기본값으로 시드해 첫 화면에서 관련 이벤트가 바로 보이게 한다. 다만 어디까지나 기본값일 뿐, 사용자는 검색어를 지워 **전체 목록을 제한 없이** 탐색·선택할 수 있다(종목 미연결 메모에서도 동일).
- **복수 선택 / 중복**: 여러 이벤트를 선택해 칩으로 누적 표시. 한 이벤트는 여러 메모에 연결될 수 있고(N:M), 같은 메모에 같은 이벤트 중복 연결은 `(memo_id, event_id)` PK로 자연히 차단된다.
- **수정 시 동작**: 동일 모달로 기존 연결을 추가·해제할 수 있어야 한다. (현재 편집 화면은 매매이벤트 연결을 "유지만" 하고 변경하지 못한다 — 이번에 실제 추가/삭제가 반영되도록 한다.)
- **표시**: 연결된 매매이벤트는 리스트형 칩(초록, §5)과 상세 화면에 표시되며, 이 표시 레이어는 이미 구현돼 있다.

### 6.5 매수/매도 폼 통합
- 매수/매도 입력 폼에 "메모 작성" 옵션 → 저장 시 메모 생성 + 해당 이벤트 및 종목에 자동 연결.
- 기존 이벤트에 나중에 메모를 붙이는 경로는 **메모 작성/편집 화면의 "매매이벤트" 선택(§6.4)**으로 제공한다.
