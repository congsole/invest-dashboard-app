...
필터 항목은 상단에 나열되며 복수 선택 가능하다. 결합 규칙은 다음과 같다.

- **같은 타입 내 복수 선택 → OR**: 종목 A + 종목 B 선택 시 둘 중 하나라도 연관된 메모를 모두 표시. 섹터도 동일.
- **다른 타입 간 → AND**: 종목 + 매매 연관 선택 시 "해당 종목 관련 메모 중 매매이벤트도 걸려있는 것"만 표시.

예시:
- 종목 A + 종목 B + 매매 연관 → `(종목 A 관련 OR 종목 B 관련) AND 매매이벤트 연결`
- 섹터 IT + 섹터 금융 + 뉴스 연관 → `(IT 섹터 OR 금융 섹터) AND 뉴스 연결`

필터 항목:
- **종목별** (복수 선택 가능) — 4.1 규칙 적용. 종목 선택 시 "직접 연결만" 토글은 표시하지 않음
- **매매 연관** — 매매이벤트에 연결된 메모만
- **뉴스 연관** — 뉴스에 연결된 메모만
- **섹터별** (복수 선택 가능) — 해당 섹터에 연결된 메모만
- **연결 없음** — 엔티티 연결이 없는 일반 메모만. 다른 필터와 상호 배타적으로 동작하며, 선택 시 다른 필터는 자동 해제됨
