...
`memo_sectors.sector_id`는 **어떤 레벨(L1~L4)이든 가리킬 수 있다.** 메모 작성 시 사용자가 L1~L4 중 원하는 단계에서 섹터/산업을 연결할 수 있으며, 데이터 레벨에서 레벨 제한을 두지 않는다.

#### 메모-섹터 연결 규칙
- 메모 작성/편집 시, L1 → L2 → L3 → L4 cascading select로 원하는 단계까지 선택 후 연결.
- 예: "정보기술"(L1)만 연결해도 되고, "반도체제조"(L4)까지 내려가서 연결해도 됨.
- 하위 단계를 선택하지 않고 중간 단계에서 확정하면 해당 레벨의 sector_id가 저장됨.
...
**변경**: L1 → L2 계층 탐색 (L3·L4는 필터 UI에 노출하지 않음).
...
- L2 일부 선택 시 → L1 칩 테두리만 색상 표시 (partial 상태).
- L2 전체 선택 시 → L1 칩 배경 채움 (all 상태).
- Level 1과 Level 2 동시 선택 가능 (OR 결합, 기존 규칙 동일).

**필터 결과 범위** (§3.4 계층 탐색 규칙 참조):
- 선택된 섹터의 하위 레벨에 속하는 **종목과 연결된 메모** + 해당 섹터/하위에 **직접 연결된 메모**를 합산(OR).

> 필터 쿼리 시 종목의 `sector_id`에서 parent를 타고 올라가 선택된 L1·L2에 속하는지 판단하고, `memo_sectors`의 직접 연결도 함께 조회한다.
