...
### Price
일별 종가 캐시(EOD). 매일 cron으로 신규 종가만 수집하며, 대시보드 진입 시 현재가 baseline으로 즉시 표시하는 데 사용된다. 실시간 현재가 조회는 별도 외부 API(Twelve Data / 네이버 / CoinGecko)를 통해 점진 갱신한다. DB 테이블명: `prices`.
...
대시보드 진입·새로고침 시 현재가를 두 단계로 표시한다.

1. **즉시 표시 (baseline)**: `prices` 테이블의 종목별 최신 종가(EOD)를 먼저 읽어 카드/KPI를 즉시 렌더링한다. 화면 전체를 스피너로 비우지 않는다. 이 값은 종가 기준 잠정치이며 갱신 시각(거래일)과 함께 표시한다.
2. **실시간 점진 갱신**: 이어서 실시간 현재가를 조회해 도착하는 대로 카드/KPI를 갱신한다. Twelve Data 무료 플랜은 분당 8크레딧(심볼 1개 = 1크레딧) 제약이 있으므로, 미국주식은 8개씩 청크로 나눠 순차 호출한다(청크 사이 약 60초 대기). 한국주식(네이버)·코인(CoinGecko)은 해당 제약이 없어 첫 청크에 함께 조회한다. 일별 종가 수집 cron도 미국주식은 8개씩 끊어 수집한다.
3. **실패 처리**: 특정 종목의 실시간 조회 실패 시 1단계의 저장 종가를 그대로 유지한다(빈칸으로 두지 않는다). 갱신 시각을 표시한다.

> 자동 폴링 없음. 현재가 호출은 화면 진입 1회 + 사용자 새로고침마다 1회.
...
| [PRD-002] 미장 가격 조회 원칙 (eabdfae) | 엔터티·속성·관계 변경 없음. Price 엔터티 설명에 "EOD baseline 즉시 표시 + 실시간 점진 갱신" 역할 명시. 계산 규칙 섹션에 "현재가 로딩 전략" 추가: (1) prices 테이블 최신 종가를 baseline으로 즉시 렌더링 — 화면 전체 스피너 금지, (2) 실시간 점진 갱신 — Twelve Data 분당 8크레딧 제약으로 미국주식 8개씩 청크 순차 호출(청크 사이 약 60초 대기), 한국주식·코인은 첫 청크에 함께 조회, cron도 동일 청크 방식, (3) 실패 시 저장 종가 유지 + 갱신 시각 표시. 자동 폴링 없음 — 진입 1회 + 새로고침 1회. |
