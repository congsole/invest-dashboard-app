...
모든 이슈의 BE/FE 트랙이 합격하면, FE 변경이 있는 이슈마다 오케스트레이터가 직접 `issues/{NNN}/09-manual-test.md`를 작성한다:
...
- PM 단계: `[PM] 이슈 생성 시작` / `[PM] 이슈 {N}개 생성: {번호들}` 또는 `[PM] 이슈 없음 — 종료`
- 구현 단계: `[{이슈번호}/{BE|FE}] {agent-name} 시작` / `[{이슈번호}/{BE|FE}] {agent-name} ✅` / `[{이슈번호}/{BE|FE}] {agent-name} ❌ ({사유})`
- 리뷰 재시도: `[{이슈번호}/{BE|FE}] {agent-name} ❌ ({N}/3) 재수정 중`
- 에러 중단: `[에러] {간단한 사유}`
