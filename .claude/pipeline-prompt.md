...
### [4] 수동 테스트 시나리오 작성 후 완료
모든 이슈의 BE/FE 트랙이 합격하면, FE 변경이 있는 이슈마다 오케스트레이터가 직접 `issues/{issue-id}/09-manual-test.md`를 작성한다:
- issue.md와 07-fe-impl.md를 읽고, 개발자가 시뮬레이터에서 직접 수행할 테스트 시나리오를 작성
- 시나리오 형식: 사전 조건 / 단계 / 기대 결과 (CLAUDE.md의 수동 테스트 섹션 참조)
- `docs/testing/manual-test-scenarios.md`(전체 기능 회귀 시나리오)에도 해당 기능 시나리오를 추가/갱신

작성 후 파이프라인을 종료한다. 테스트 수행 자체는 개발자가 직접 한다 (자동화 없음).
...
파이프라인 진행 상황을 `logs/pipeline-status.log`에 기록한다.
각 에이전트 시작/완료 시점에 아래 형식으로 **한 줄씩** append한다:
...
### 형식 규칙
- 파이프라인 시작/종료: `── 파이프라인 시작 ({커밋해시 앞7자리})` / `── 파이프라인 완료 ✅` / `── 파이프라인 실패 ❌`
- 설계 단계: `[설계] {agent-name} 시작` / `[설계] {agent-name} ✅`
- PM 단계: `[PM] 이슈 생성 시작` / `[PM] 이슈 {N}개 생성: {ID들}` 또는 `[PM] 이슈 없음 — 종료`
- 구현 단계: `[{슬러그}/{BE|FE}] {agent-name} 시작` / `[{슬러그}/{BE|FE}] {agent-name} ✅` / `[{슬러그}/{BE|FE}] {agent-name} ❌ ({사유})`
- 리뷰 재시도: `[{슬러그}/{BE|FE}] {agent-name} ❌ ({N}/3) 재수정 중`
- 에러 중단: `[에러] {간단한 사유}`

### 주의
- 상세 내용은 이슈 파일에 이미 기록되므로, 현황 로그는 **한 줄 요약**만 쓴다
- 서브에이전트에게 이 로그를 쓰라고 지시하지 않는다 — 오케스트레이터(메인)가 에이전트 호출 전후에 직접 쓴다
