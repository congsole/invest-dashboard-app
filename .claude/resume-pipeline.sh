...
" \
  ...
  if [ $EXIT_CODE -eq 0 ]; then
    osascript -e "display notification \"파이프라인 완료\" with title \"invest-dashboard\" sound name \"Glass\""
  else
    osascript -e "display notification \"파이프라인 에러로 중단됨 — .claude/pipeline.log 확인\" with title \"invest-dashboard\" sound name \"Basso\""
  fi
) &
